...
public class FoodPayStatusPresenter {
    private IPresenterBase<List<VideoPayBean>> iPresenterBase;
    private Context mContext;

    public FoodPayStatusPresenter(Context mContext, IPresenterBase<List<VideoPayBean>> iPresenterBase) {
        this.mContext = mContext;
        this.iPresenterBase = iPresenterBase;
    }

    public void request(Object tag, String orderNum) {
        HttpParams params = new HttpParams();
        params.put("orderNum", orderNum);
        OkHttpUtils.postJson(HttpConstants.HTTP_HOST + HttpConstants.GET_FOOD_PAY_STATUS, tag, params, new JsonCallback<BaseModel<List<VideoPayBean>>>() {
            @Override
            public void onSuccess(Response<BaseModel<List<VideoPayBean>>> response) {
                BaseModel<List<VideoPayBean>> body = response.body();
                if (body.code == 1) {
                    Log.e("FoodPayStatusPresenter",body.msg);
            ...
            @Override
            public void onError(Response<BaseModel<List<VideoPayBean>>> response) {
                iPresenterBase.onError();
            }
        });
    }
