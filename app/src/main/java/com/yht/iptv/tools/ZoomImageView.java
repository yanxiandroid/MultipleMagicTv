...
public class ZoomImageView extends AppCompatImageView {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        Toast.makeText(getContext(), event.getRawX() + "--" + event.getRawY(), Toast.LENGTH_SHORT).show();
            ...
            case MotionEvent.ACTION_MOVE:
                if (event.getPointerCount() == 1) {
                    ...
                    if ((currentStatus == STATUS_ZOOM_OUT && totalRatio < 4 * initRatio)
                        ...
                        scaledRatio = (float) (2 / lastFingerDis);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (event.getAction() == KeyEvent.ACTION_DOWN && event.getKeyCode() == KeyEvent.KEYCODE_DPAD_DOWN) {
            if(zoomNum == 2) {
                if(totalTranslateY < - height){
                    Log.e("zoomimageview","到达底部");
                    return true;
                }
                Log.e("zoomimageview",totalTranslateX + "----" + totalTranslateY   + "currentBitmapWidth"+currentBitmapWidth + "---" +"" + currentBitmapHeight);
                currentStatus = STATUS_MOVE;
                movedDistanceX = 0;
                movedDistanceY = -20;
                // 调用onDraw()方法绘制图片
...
//            // 只有单指按在屏幕上移动时，为拖动状态
//            float xMove = location[0];
//            float yMove = location[1];
//            if (lastXMove == -1 && lastYMove == -1) {
//                lastXMove = xMove;
//                lastYMove = yMove;
//            }
...
//            }
            if(zoomNum == 2) {
                if(totalTranslateX < - width){
                    Log.e("zoomimageview","到达最右边");
                    return true;
                }
                Log.e("zoomimageview",totalTranslateX + "----" + totalTranslateY   + "currentBitmapWidth"+currentBitmapWidth + "---" +"" + currentBitmapHeight);
                currentStatus = STATUS_MOVE;
