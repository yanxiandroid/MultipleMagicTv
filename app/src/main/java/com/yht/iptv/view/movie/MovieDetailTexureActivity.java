...
public class MovieDetailTexureActivity extends BaseActivity implements TvRecyclerView.OnItemListener, MediaPlayer.OnInfoListener, MediaPlayer.OnErrorListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener, TextureView.SurfaceTextureListener {
...
//    private VideoView movie_video;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_movie_detail_texure);
    ...
    private void initView() {
        ...
        ViewGroup.LayoutParams layoutParams1 = mTextureView.getLayoutParams();
        layoutParams.height = screenWeight * 9 / 16;
        mTextureView.setLayoutParams(layoutParams1);
    ...
    private void initData() {
        ...
        detailPresenter = new MediaDetailPresenter(MovieDetailTexureActivity.this, new DetailListener());
...
//            presenter.request(this, 15, roomId);
        ...
        if (Constants.mainPageInfo.getAdSetting().getMediaDetailImage() == 1) {
            PictureAdPresenter pictureAdPresenter = new PictureAdPresenter(MovieDetailTexureActivity.this, new PictureAdListener());
        ...
        adapter = new MediaRecyclerViewAdapter(MovieDetailTexureActivity.this, detailList);
    ...
    private void videoPayStatus() {
        if (detailList.get(currentPosition).getVideoPayment() == 1) {
            if (detailList.get(currentPosition).getPrice() > 0f) {
                if (isenterEnable) {
                    initDialog(MovieDetailTexureActivity.this, getStrings(R.string.loading));
                }
...
//                movie_video.start();
    ...
    private void adStatistics() {
        try {
            AdvertInfo advertInfo = DBUtils.find(MovieDetailTexureActivity.this, AdvertInfo.class, "id", "=", FileUtils.getAdvert15Path() + Constants.key[advertPosition % Constants.key.length]);
            if (advertInfo != null) {
            ...
            switch (msg.what) {
                case PLAY://预览视频播放
                    ...
                    try {
                        ...
                        playTime = previewBegin + previewLength;
                    }
...
//        }
    ...
    private void ClickData() {
        ...
        if (permissions) {
            ...
            VideoPayPresenter videoPayPresenter = new VideoPayPresenter(MovieDetailTexureActivity.this, new VideoPayListener());
            videoPayPresenter.request(roomId, info.getCustomerId(), 0, detailList.get(currentPosition).getId());
        }
    }
...
//            }
        ...
        public void onError() {
            ...
            tv_advert_msg.setVisibility(View.GONE);
            try {
                ShowImageUtils.showImageView(MovieDetailTexureActivity.this, Constants.mainPageInfo.getAdSetting().getMediaDetailImageFile().getPath(), nothing_iv);
            } catch (Exception e) {
                ShowImageUtils.showImageView(MovieDetailTexureActivity.this, R.drawable.main_bg, nothing_iv);
            }
            overridePendingTransition(R.anim.act_switch_fade_in, R.anim.act_switch_fade_out);
        }
    }
    ...
    private class MyAdvertListener implements IPresenterDownloadBase<List<AdvertVideoInfo>> {
        ...
        public void onDownLoadSuccess(String fileName) {
            ...
            AdvertInfo advertInfo = DBUtils.find(MovieDetailTexureActivity.this, AdvertInfo.class, "id", "=", id);
            if (advertInfo == null) {
                advertInfo = new AdvertInfo();
                advertInfo.setId(id);
                advertInfo.setCurrentAdvId(advertId);
                DBUtils.save(MovieDetailTexureActivity.this, advertInfo);
            } else {
                advertInfo.setCurrentAdvId(advertId);
                DBUtils.update(MovieDetailTexureActivity.this, advertInfo, "currentAdvId");
            }
            if (!isAdvPlayFirst) {
                isAdvPlayFirst = true;
                initVideoView();
...
//                movie_video.start();
    ...
    private static class MyHandler extends Handler {
        ...
        @Override
        public void handleMessage(Message msg) {
            MovieDetailTexureActivity activity = (MovieDetailTexureActivity) weakReference.get();
    ...
    private void showPayMentChargeDialog(final VideoPayBean videoPayBean) {
        if (payMentChargeDialog == null || !payMentChargeDialog.isShowing()) {
            payMentChargeDialog = new PayMentChargeDialog(MovieDetailTexureActivity.this, new IDialogListener() {
                @Override
    ...
    private void setMediaPayDialog(int payment, VideoPayBean videoPayBean) {
        if (payDialog == null) {
            try {
                ...
                payDialog = new PayDialog(MovieDetailTexureActivity.this, payment, bitmap
...
//        movie_video.stopPlayback();
     ...
     * 播放
     */
    public void play(String localUrl,int position) {
        try {
            if(mediaPlayerState != 1){
                initMediaPlayer();
            }
